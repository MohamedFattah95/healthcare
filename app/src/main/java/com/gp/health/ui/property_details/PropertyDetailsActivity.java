...
import com.gp.health.databinding.ActivityPropertyDetailsBinding;
import com.gp.health.databinding.DialogReportBinding;
...
public class PropertyDetailsActivity extends BaseActivity<PropertyDetailsViewModel> implements PropertyDetailsNavigator, OnMapReadyCallback, MemberAdsAdapter.Callback {
    ...
    private void subscribeViewModel() {
        ...
        mViewModel.getItemDetailsLiveData().observe(this, response -> {
            ...
            if (mViewModel.getDataManager().isUserLogged() && itemDetails.getUser() != null &&
                    itemDetails.getUser().getId() == mViewModel.getDataManager().getUserObject().getId()) {
            ...
            if (response.getData().getUser() != null) {
                ...
                Glide.with(this)
                        .load(response.getData().getUser().getImage())
                        .placeholder(R.drawable.ic_user_holder)
                        .error(R.drawable.ic_user_holder)
                        .into(binding.userImageView);

                if (response.getData().getUser().getGrade() == 0)
                    Glide.with(this)
                            .load("")
                            .into(binding.ivLevel);
                else if (response.getData().getUser().getGrade() == 1)
                    Glide.with(this)
                            .load(R.drawable.ic_bronze)
                            .into(binding.ivLevel);
                else if (response.getData().getUser().getGrade() == 2)
                    Glide.with(this)
                            .load(R.drawable.ic_silver)
                            .into(binding.ivLevel);
                else if (response.getData().getUser().getGrade() == 3)
                    Glide.with(this)
                            .load(R.drawable.ic_gold)
                            .into(binding.ivLevel);
                else if (response.getData().getUser().getGrade() == 4)
                    Glide.with(this)
                            .load(R.drawable.ic_platinum)
                            .into(binding.ivLevel);

                binding.ratingBarClient.setRating(response.getData().getUser().getRate());
                binding.tvRateCount.setText("(" + response.getData().getUser().getRateCount() + ")");


                binding.tvJoinDate.setText(getString(R.string.join_at) + " " + DateUtility.getDateOnlyTFormat(response.getData().getUser().getCreatedAt()));
    ...
    public void setUpOnViewClicked() {
        ...
        binding.llMember.setOnClickListener(v -> {
            if (itemDetails != null && itemDetails.getUser() != null) {
                if (mViewModel.getDataManager().isUserLogged()) {
                    if (mViewModel.getDataManager().getUserObject().getId() == itemDetails.getUser().getId()) {
                        startActivity(MyProfileActivity.newIntent(this));
                    } else {
                        startActivity(MemberProfileActivity.newIntent(this).putExtra("userId", itemDetails.getUser().getId()));
                    }
                } else {
                    startActivity(MemberProfileActivity.newIntent(this).putExtra("userId", itemDetails.getUser().getId()));
                }
            ...
            if (mViewModel.getDataManager().isUserLogged()) {
                if (itemDetails.getUser() != null) {
                    startActivity(ChatActivity.newIntent(this)
                            .putExtra("receiver_id", itemDetails.getUser().getId()));
                }
            } else {
                CommonUtils.handleNotAuthenticated(this);
            }
        ...
        binding.tvCall.setOnClickListener(v -> {
            ...
            if (mViewModel.getDataManager().isUserLogged()) {
                if (itemDetails.getUser() != null)
                    startActivity(new Intent(Intent.ACTION_DIAL).setData(Uri.parse("tel:" + itemDetails.getUser().getMobile())));
            } else {
                CommonUtils.handleNotAuthenticated(this);
            }
