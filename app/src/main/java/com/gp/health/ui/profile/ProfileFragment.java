...
import com.gp.health.databinding.FragmentProfileBinding;
...
public class ProfileFragment extends BaseFragment<ProfileViewModel> implements ProfileNavigator {
    ...
    public void updateProfile(String keyword) {
        if (keyword.equalsIgnoreCase("update_profile")) {
            if (mViewModel.getDataManager().isUserLogged()) {
                binding.tvUsername.setText(mViewModel.getDataManager().getUserObject().getName());
                Glide.with(this)
                ...
                binding.ratingBarClient.setRating(mViewModel.getDataManager().getUserObject().getRate());
                binding.tvRateCount.setText("(" + mViewModel.getDataManager().getUserObject().getRateCount() + ")");

                binding.tvJoinDate.setText(getString(R.string.join_at) + " " + DateUtility.getDateOnlyTFormat(mViewModel.getDataManager().getUserObject().getCreatedAt()));

                binding.tvQuoteDetails.setText(mViewModel.getDataManager().getUserObject().getDescription());
