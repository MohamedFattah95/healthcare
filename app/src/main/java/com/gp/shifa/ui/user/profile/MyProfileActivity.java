...
import com.gp.shifa.databinding.ActivityMyProfileBinding;
...
public class MyProfileActivity extends BaseActivity<ProfileViewModel> implements ProfileNavigator {
    ...
    private void subscribeViewModel() {
        ...
        mViewModel.getProfileLiveData().observe(this, response -> {
            ...
            binding.toolbar.toolbarTitle.setText(response.getUser().getName());

            binding.tvUsername.setText(response.getUser().getName());
            Glide.with(this)
                    .load(response.getUser().getImgSrc()
                            + "/" + response.getUser().getImg())
                    .placeholder(R.drawable.ic_user_holder)
                    .error(R.drawable.ic_user_holder)
                    .into(binding.userImageView);

            binding.tvQuoteDetails.setText(response.getUser().getGander());

            RenderOption renderOption = new RenderOption();
            renderOption.setContent(String.valueOf(response.getUser().getId())); // content to encode
